    ...
    <div className={className}>
      {regions && regions.length > 0 && (
        <div className="relative hidden lg:flex w-[76px]" ref={containerRef}>
          ...
          <div
            className={`md:w-18 overflow-hidden rounded-[10px] bg-white shadow-[0px_7px_40px_0px_rgba(0,0,0,0.10)] absolute top-[calc(100%+4px)] left-0 right-0 flex-col ${openSwitcher ? 'flex' : 'hidden'}`}
          >
            {regions.map((region) =>
              currentRegion === region.locale ? (
                <div
                  key={region.locale}
                  className="flex gap-2 items-center opacity-80 p-[10px] border-b border-zinc-200 last:border-none"
                >
                  <ImageLoader
