...
export const EmbedForm: React.FC<EmbedFormProps> = ({
  ...
  const formId =
    data?.embedForm?.formId ||
    data?.formId ||
    (siteSettings && siteSettings[0]?.demoMainFormId
      ? siteSettings[0]?.demoMainFormId
      : '6b2d6906-028e-4d65-9cd1-34d528e0d5c0')
  const meetingLink =
    data?.embedForm?.meetingLink ||
    data?.meetingLink ||
    (siteSettings && siteSettings[0]?.demoMeetingLink
      ? siteSettings[0]?.demoMeetingLink
      : undefined)
  const eventName =
    data?.embedForm?.eventName ||
    data?.eventName ||
    (siteSettings && siteSettings[0]?.dmeoFormEventName
      ? siteSettings[0]?.dmeoFormEventName
      : undefined)
  ...
  return (
    <div
      ...
      aria-modal="true"
    >
      <div className="fixed  transition-opacity" aria-hidden="true"></div>
