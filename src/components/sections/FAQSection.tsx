...
const FAQSection: React.FC<FAQSectionProps> = ({ faqCategories = [] }) => {
  ...
  // Multiple categories: reset accordion when switching. Single category: open first Q&A.
  useEffect(() => {
    ...
    const cat = faqCategories.find((c) => c._id === activeCategory)
    const qs = cat?.faq?.faqs || []
    if (qs.length > 0) {
      setIsOpen({ [`${activeCategory}-0`]: true })
    }
  }, [activeCategory, hasMultipleCategories, faqCategories])
