  ...
  if (req.method === 'POST') {
    ...
    try {
      ...
      if (!email || !pageUrl) {
        res
          .status(400)
          .json({ err: 'Missing email or page_url in query params' })
        return
      }
      const isPlaybookDownload =
        sidebarTitle &&
        sidebarTitle.toLowerCase().includes('download this resource for free')
      ...
      if (isPlaybookDownload) {
        ...
        var raw = JSON.stringify({
          properties: {
            playbook_downloads: playbook_downloads
              ? `${playbook_downloads}, ${pageUrl}`
              : pageUrl,
          },
        })
