...
const EmbedFormSection: React.FC<EmbedFormSectionProps> = ({
  ...props
}: any) => {
  ...
  const meetingLink =
    embedFormdata?.meetingLink || embedFormdata?.embedForm?.meetingLink
  const eventName =
    embedFormdata?.eventName || embedFormdata?.embedForm?.eventName
  const videoLink =
    embedFormdata?.videoLink || embedFormdata?.embedForm?.videoLink
  const sidebarTitle =
    embedFormdata?.sidebarTitle || embedFormdata?.embedForm?.sidebarTitle
  ...
  return (
    <div
      id={`#hubspotForm`}
      className={` not-prose flex flex-col gap-6  ${!isMobile && openForm ? 'h-[400px]' : ''}`}
    >
      <SidebarTitle className=" !text-3xl border-zinc-200 pt-6 pb-2">
        {embedFormdata?.sidebarTitle}
      </SidebarTitle>
      <EmbedForm
        ...
        onSubmitSuccess={() => {
          if (pdfUrl) {
            download_file(
              pdfUrl,
              `${embedFormdata?.title?.originalFilename ? embedFormdata?.title?.originalFilename : name}`,
            )
          }
        }}
      />
