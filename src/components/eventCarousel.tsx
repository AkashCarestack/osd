...
const EventCarousel = ({
  ...
  return (
    homeSettings?.eventCarousel && (
      <Section
        className={`justify-center ${bgColor ? bgColor : 'bg-zinc-900'}  px-4`}
      >
            ...
            {allEventCards &&
              allEventCards.map((events) => {
                return (
                  ...
                  >
                    <div
                      className={`w-full ${bgColor ? bgColor : 'bg-zinc-900'} `}
                    >
                      <div
                        style={{
                          backgroundColor: `${events.bgColor ? events.bgColor : '#000000'}`,
                        }}
                      ...
                      >
                        ...
                        <div className="flex flex-col flex-1 gap-4 self-center w-full p-4 md:p-0 md:py-5">
                          {events.eventDescription && (
                            <div className="border-b border-[#33333333] pb-4">
                              <H4Large
                                className={`group-hover: group-hover: line-clamp-3 text-ellipsis overflow-hidden  xl:!text-xl`}
                              >
                                {events.eventDescription}
                              </H4Large>
                            </div>
                          ...
                          <div className=" flex xl:flex-row gap-x-6 gap-y-3 flex-col justify-between items-start xl:items-center ">
                            <div className="flex items-start md:items-center gap-3 xl:gap-6 flex-col md:flex-row ">
                              <div
                                className={`!bg-[#18181B33] flex flex-shrink-0 pt-[7px] pb-[7px] pl-[10px] pr-[10px] justify-center items-center gap-3 rounded-full  bg-opacity-20 `}
                              >
                                <SubText className="!text-white">
                                  {events.evenTtype == 'Online Event'
                                    ? 'Live Event'
                                    : events.evenTtype}
                                </SubText>
                              </div>
                            ...
                            <div className="self-auto">
                              <div className="self-start md:self-center flex justify-center">
                                <Button
                                  className="bg-zinc-900 transition-all duration-300 ease-in-out hover:bg-zinc-700 !no-underline"
                                  target="_blank"
                                  link={
                        ...
                        <div className="flex flex-row md:flex-col items-center justify-center gap-2 md:gap-4 z-50 p-6 pt-0 md:pb-0">
                          {allEventCards &&
                            allEventCards.length > 1 &&
                            allEventCards.map((_, index) => (
                              <button
                                key={index}
                                onClick={() =>
