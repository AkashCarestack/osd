...
const RelatedTag = ({ tags, className }: RelatedTagProps) => {
  ...
  return (
    <Section
      className={`justify-center border-t border-zinc-200 !p-0 !pt-4 !pb-0 md:!mt-4  ${className}`}
    >
      <div className="w-full  md:flex-row flex  flex-wrap md:gap-4 gap-2">
        ...
        {tags &&
          ...
          tags.map((tag: any, index) => {
            ...
            return (
              <Anchor
                className="rounded-md bg-zinc-100  md:px-4 md:py-[5px]  px-2 py-[3px]   hover:bg-zinc-200 transition-all duration-300 ease-in-out"
                href={hrefLink}
                key={tag?.slug?.current}
              >
                <DescriptionText
                  className={`text-zinc-600 font-normal  md:text-base text-xs`}
                >
                  {tag.tagName}
                </DescriptionText>
              </Anchor>
