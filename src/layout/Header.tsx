...
export const navigationLinks = [
  { id: 'training', label: 'Training', sectionId: 'knowledge-guides-section' },
  {
    ...
    sectionId: 'events-updates-section',
  },
...
const Header = () => {
  ...
  const before =
    "before:content-[''] before:h-[100px] before:absolute before:left-0 before:right-0 before:top-full before:bg-vs-blue"
  return (
    <>
      {/* <ProgressBar /> */}
      <div
        className={`relative w-full before:content-[''] before:-z-0  before:absolute before:left-0 before:right-0 before:top-[-100px] before:bg-zinc-900`}
      >
        <header
                  ...
                  >
                    {!isMobile && (
                      ...
                      >
                        <Link
                          href={topicsUrl}
                          className="text-zinc-500 flex items-center gap-[6px] cursor-pointer text-base hover:text-zinc-300"
                        >
                          <span>{`Topics`}</span>
                          <ChevronDownIcon
                            className={`w-5 h-5 transition-transform duration-300 ease-in-out ${showTopicsDropdown ? 'rotate-180' : ''}`}
                          />
                        </Link>
                        {navPopoverId && (
                          <ShortNavPopover
                            navPopoverId={navPopoverId}
                            showMenu={showTopicsDropdown}
                            setShowMenu={setShowTopicsDropdown}
                            className="z-10 lg:group-hover:block lg:group-hover:visible lg:group-hover:opacity-100"
                          />
                        )}
                      </div>
                    )}
